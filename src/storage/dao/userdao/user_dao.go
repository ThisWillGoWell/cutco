...
import (
	"context"
	"cutco-camper/src/errors"
	"cutco-camper/src/models"
	"cutco-camper/src/starketext"
	"cutco-camper/src/storage/ddb"
	"encoding/json"
	"fmt"
	"go.uber.org/zap"
)
...
type usersTable struct {
	table *ddb.SingleTable
}

func (t usersTable) LoadUserByID(ctx context.Context, userID string) (*models.User, error) {
	pk, sk := models.User{ID: userID}.Keys()
	user := &models.User{}
	return user, ddb.LoadItemByKey(ctx, t.table, user, pk, sk)
}

func (t usersTable) LoadAccountByID(ctx context.Context, userID string) (*models.Account, error) {
	pk, sk := models.Account{UserID: userID}.Keys()
	account := &models.Account{}
	return account, ddb.LoadItemByKey(ctx, t.table, account, pk, sk)
}

func (t usersTable) LoadAccountByLogin(ctx context.Context, login string) (*models.Account, error) {
	pk, _ := models.Account{Login: login}.GSI0()
	account := &models.Account{}
	return account, ddb.LoadItemByQuery(ctx, t.table, account, ddb.QueryIndexInput{
		IndexName: ddb.GSI0Name,
		IndexPk:   pk,
	})
}

func (t usersTable) LoadAccountAndUserById(ctx context.Context, userID string) (*models.Account, *models.User, error) {
	pk, userSk := models.User{ID: userID}.Keys()
	_, accountSk := models.Account{UserID: userID}.Keys()

	user, account := &models.User{}, &models.Account{}

	return account, user, ddb.LoadItemsByQuery(ctx, t.table, func(sk string, jsonStr []byte) error {
		switch sk {
		case userSk:
			return json.Unmarshal(jsonStr, user)
		case accountSk:
			return json.Unmarshal(jsonStr, accountSk)
		}
		return nil
	}, ddb.QueryIndexInput{IndexPk: pk, IndexSkPrefix: models.InfoPrefix})
}

func (t usersTable) LoadAccountByUserID

func (table *usersTable) CreateNewUser(ctx context.Context, userPrivate *models.PrivateUserStruct) error {

	// attempt to create a userdao with a random id,
...
//	userdao, err := table.Users.loadUser(ctx, request)
//	if err != nil {
...
func (table *usersTable) LoadUsers(ctx context.Context, request models.ReadUsersRequest) ([]*models.UserStruct, error) {
	...
	if request.UserIDs != nil && len(request.UserIDs) == 0 {
		...
		return nil, fmt.Errorf("missing userdao lookup values")
	} else if request.UserIDs != nil {
		logger.Debugw("load users by userdao-ids")
