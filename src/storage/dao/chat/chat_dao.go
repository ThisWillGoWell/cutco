...
// given two userdao ids, load a whisper entry
...
func (table *chatTable) loadChatChannels(ctx context.Context, request models.ReadChatRequest) ([]*models.ChatChannelStruct, error) {
	...
	if request.UserID != "" {
		// chat membership is stored as userdao#UserID / chat#ChatID
	...
	if request.Selects.UserSelects != nil {
		// load all the userdao IDs
		// map of userdao id to index of channel they are in
